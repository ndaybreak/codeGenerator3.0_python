function OptSqCustExcpEditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	...
	var attrList = ['customerId', 'idType', 'note', 'lastModById', 'lastModTs'];
	var forDisplayList = ['customerId', 'idType', 'note', 'lastModByEmail', 'lastModTs'];
	//finished ajax call
	var count = 0;
	...
	AdminService.getOptSqCustExcpData(para,function(result) {
		...
		$scope.headers = [
			...
			{title: 'Customer ID',value: 'customerId',cd: 'customerId',visible: true,className: 'pg-col-width'},
			{title: 'ID type',value: 'idType',cd: 'idType',visible: true,className: 'pg-col-width'},
			{title: 'Note',value: 'note',cd: 'note',visible: true,className: 'pg-col-width'},
			{title: 'Updated By',value: 'lastModByEmail',cd: 'lastModById',visible: true,className: 'pg-col-width'},
			{title: 'Updated On',value: 'lastModTs',cd: 'lastModTs',visible: true,className: 'pg-col-width'},
    	];
		$scope.currentPage = 1;
		pgOptions.data = result.lineData;
		pgOptions.totalSize = result.totalRecordAmount;
