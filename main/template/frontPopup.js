...
{% for item in xc.items %}
	{% if item.checks and item.checks|length > 0 %}
...
{% endfor %}
		...
		{
			loading.open();
			AdminService.save{{ xc.entity_name }}(para, function({{ xc.lowerCaseEntityName() }}List){
				{{ xc.lowerCaseEntityName() }}List.totalRecordAmount = 1 + {{ xc.lowerCaseEntityName() }}List.totalRecordAmount; 
				sharedService.shareGridData({{ xc.lowerCaseEntityName() }}List,'{{ xc.lowerCaseEntityName() }}');
		...
		else if ($scope.action === ACTION_UPDATE)
		{
			...
			AdminService.update{{ xc.entity_name }}(para, function({{ xc.lowerCaseEntityName() }}List){
				sharedService.shareGridData({{ xc.lowerCaseEntityName() }}List,'{{ xc.lowerCaseEntityName() }}');
				loading.close();
