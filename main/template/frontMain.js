function {{ xc.entity_name }}EditDataGridCtrlPg($scope, $filter, $modal, $document, $window, AdminService, loading, confirm, logger, formatData)
{
	...
	$scope.update = function () {
		...
		$modal.open({templateUrl: "html/{{ xc.entity_name }}Edit.html", controller: {{ xc.entity_name }}EditCtrl, {{ xc.jsUseTwoCols() }}
								keyboard: false,
	...
	AdminService.get{{ xc.entity_name }}Data(para,function(result) {
		...
		$scope.headers = [
		{% for colm in xc.colms_info %}
			{title: '{{ colm[0] }}',value: '{{ colm[1] }}',cd: '{{ colm[2] }}',visible: {{ xc.lowerCaseVisible(colm[3]) }},className: '{{ colm[4] }}'},
		{% endfor %}
    	];
		$scope.currentPage = 1;
		pgOptions.data = result.lineData;
