...
{% for entity in xc.entity_list %}
	//{{ entity.lowerCaseEntityName() }}
	var {{ entity.lowerCaseEntityName() }}FilteredData,
	    {{ entity.lowerCaseEntityName() }}SelectData;
{% endfor %}
	   ...
	   //---{{ entity.entity_name }}
		get{{ entity.entity_name }}SelectData: function(callback)
		{
			...
			this.httpGet({{ entity.select_url }},
									function(result)
								   {
								        ...
								        logger.debug(JSON.stringify(result.data));
								        {{ entity.lowerCaseEntityName() }}SelectData = result.data;
								        callback && callback({{ entity.lowerCaseEntityName() }}SelectData);
								   }, 
		...
		get{{ entity.entity_name }}Data: function(para,callback)
		{
			...
			this.httpGet({{ entity.no_filter_url }} +'/'+para.pageSize+'/'+para.pageIndex,function(result){
								        ...
								        logger.debug(JSON.stringify(result.data));
								        {{ entity.lowerCaseEntityName() }}FilteredData = result.data;
								        formatData.toDollerData({{ entity.lowerCaseEntityName() }}FilteredData.lineData,para.dollerFields);
								        formatData.toPercentData({{ entity.lowerCaseEntityName() }}FilteredData.lineData,para.percentFields);
								        callback && callback({{ entity.lowerCaseEntityName() }}FilteredData);
								   }, 
	...
	{% if entity.entity_name != "OptPriceQualityBand" %}
		save{{ entity.entity_name }}: function(options, callback)
		{
			...
			this.httpPost({{ entity.save_url }}, options.item, function(result){
				...
				{{ entity.lowerCaseEntityName() }}FilteredData.lineData.push(result.data);
				if(callback)callback({{ entity.lowerCaseEntityName() }}FilteredData);
			});
		},
	{% endif %}
{% endfor %}
	...
	{% if entity.entity_name != "OptPriceQualityBand" %}
		delete{{ entity.entity_name }}: function(options,callback)
		{
			...
			this.httpPost({{ entity.delete_url }}, options.itemToDelete, function(result){
				...
				{{ entity.lowerCaseEntityName() }}FilteredData = removeItemFromList({{ entity.lowerCaseEntityName() }}FilteredData, result.data, "id");
				if(callback)callback({{ entity.lowerCaseEntityName() }}FilteredData);
			});
		},
	...
	{% if entity.entity_name != "OptPriceQualityBand" %}
		update{{ entity.entity_name }}: function(options,callback)
		{
			...
			this.httpPost({{ entity.update_url }}, options.item, function(result){
				formatData.toDollerData(result.data,options.dollerFields);
				formatData.toPercentData(result.data,options.percentFields);
				sync(result.data, {{ entity.lowerCaseEntityName() }}FilteredData, "id");
